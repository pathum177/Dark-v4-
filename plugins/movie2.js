...
cmd({
  ...
  try {
    ...
    if (!searchData) {
      const searchUrl = `https://sadiya-tech-apis.vercel.app/movie/zoom-search?text=${encodeURIComponent(q)}`;
      let retries = 3;
      while (retries > 0) {
        try {
    ...
    const selectionHandler = async (update) => {
      ...
      if (repliedToId === movieListMessageKey.id) {
        ...
        const downloadUrl = `tps://suhas-bro-api.vercel.app/movie/sinhalasub/movie?url=${encodeURIComponent(selectedFilm.link)}`;
        let downloadData;
