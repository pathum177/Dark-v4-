    ...
    try {
      ...
      if (action === "on") {
        ...
        if (initialNews) {
          ...
          reply("* chama-md Warning:* Failed to fetch initial derana news. API may be down. Continuing with updates...");
        }
          ...
          try {
            ...
            if (news) {
              if (news.title !== lastSentNewsTitle) {
                await sendNews(robin, autoNewsGroupId, news, "DERANA", savedMek);
        ...
        return reply(
          ...
          "- Latest DERANA news sent (if available).\n" +
