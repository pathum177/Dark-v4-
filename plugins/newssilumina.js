  ...
  async (
    ...
    try {
      ...
      if (action === "on") {
        ...
        if (initialNews) {
          await sendNews(robin, autoNewsGroupId, initialNews, "SILUMINA", savedMek);
          lastSentNewsTitle = initialNews.title;
        } else {
          reply("* chama-md Warning:* Failed to fetch initial SILUMINA news. API may be down. Continuing with updates...");
        }
          ...
          try {
            ...
            if (news) {
              if (news.title !== lastSentNewsTitle) {
                await sendNews(robin, autoNewsGroupId, news, "SILUMINA", savedMek);
...
async function fetchNews(apiUrl) {
  try {
    ...
    if (!response.data || !response.data.status || !response.data.result) {
      console.log("Chama-md : No news found or invalid response format from API:", apiUrl);
      return null;
    }
    return response.data.result; // Expected: { title, image, date, desc, url } for Derana
  } catch (e) {
    console.error("Chama-md : Failed to fetch news from API:", apiUrl, e.response ? e.response.status : e.message);
    return null;
  }
}
